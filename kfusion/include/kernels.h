...
void integrateKernel(Volume vol, const float* depth, uint2 imageSize, const Matrix4 invTrack, const Matrix4 K,
...
void computeFrame(Volume& integration, float3* vertex, float3* normal, TrackData* trackingResult, Matrix4& pose,
                  const float* inputDepth, const uint2 inputSize, const float* gaussian,
...
class Kfusion {
  ...
  public:
    float current_matching_error;  // UT:

    Kfusion(uint2 inputSize, uint3 volumeResolution, float3 volumeDimensions, float3 initPose,
        ...
        pose = toMatrix4(TooN::SE3<float>(TooN::makeVector(initPose.x, initPose.y, initPose.z, 0, 0, 0)));
        ...
        current_matching_error = 0;
    }
    // Allow a kfusion object to be created with a pose which include orientation as well as position
    Kfusion(uint2 inputSize, uint3 volumeResolution, float3 volumeDimensions, Matrix4 initPose,
            std::vector<int>& pyramid)
        : computationSize(make_uint2(inputSize.x, inputSize.y)) {
    ...
    void reset();
    bool getTracked() { return (_tracked); }
    bool getIntegrated() { return (_integrated); }

    float getCurrentError() {  // UT: Matching error
        return current_matching_error;
    }
    ...
    void computeFrame(const ushort* inputDepth, const uint2 inputSize, float4 k, uint integration_rate,
    ...
    void compareVolume(uchar4* out, const uint2 outputSize, int frame, int rate, float4 k, float mu, const char* ref_file);
    ...
    Matrix4 updatePose(Matrix4 new_pose) { pose = new_pose; }
    ...
    Matrix4* getViewPose() { return (viewPose); }
    float3 getModelDimensions() { return (volumeDimensions); }
    uint3 getModelResolution() { return (volumeResolution); }
    uint2 getComputationResolution() { return (computationSize); }

    void updateComputationSize(uint2 cSize);  // UT:

    void updateIterations(uint3 target_pd) {
        this->iterations[0] = target_pd.x;
        this->iterations[1] = target_pd.y;
        this->iterations[2] = target_pd.z;
    }
};

...
static std::string short_options = "qc:d:f:i:l:m:k:o:p:r:s:t:v:y:z:u:w:x:";

static struct option long_options[] = {{"compute-size-ratio", required_argument, 0, 'c'},
                                       ...
                                       {"control", required_argument, 0, 'u'},
                                       {"extrapolate", required_argument, 0, 'x'},
                                       {"features", required_argument, 0, 'w'},
                                       {0, 0, 0, 0}
...
struct Configuration {
    ...
    inline void print_arguments() {
        std ::cerr << "-c  (--compute-size-ratio)       : default is " << default_compute_size_ratio
                   << "   (same size)      " << std::endl;
        ...
        std ::cerr << "-p  (--init-pose)                : default is " << default_initial_pos_factor.x << ","
                   << default_initial_pos_factor.y << "," << default_initial_pos_factor.z << "     " << std::endl;
        std ::cerr << "-q  (--no-gui)                   : default is to display gui" << std::endl;
        std ::cerr << "-r  (--integration-rate)         : default is " << default_integration_rate << "     "
                   << std::endl;
        std ::cerr << "-s  (--volume-size)              : default is " << default_volume_size.x << ","
                   << default_volume_size.y << "," << default_volume_size.z << "      " << std::endl;
        std ::cerr << "-t  (--tracking-rate)            : default is " << default_tracking_rate << "     " << std::endl;
        std ::cerr << "-v  (--volume-resolution)        : default is " << default_volume_resolution.x << ","
                   << default_volume_resolution.y << "," << default_volume_resolution.z << "    " << std::endl;
    ...
    void print_values(std::ostream& out) {
        ...
        out << "init-pose: " << initial_pos_factor.x << "," << initial_pos_factor.y << "," << initial_pos_factor.z
            << std::endl;

        out << std::endl;
        out << "Algorithmic properties:" << std::endl << "=======================" << std::endl << std::endl;
        out << "compute-size-ratio: " << compute_size_ratio << std::endl;
        out << "volume-resolution: " << volume_resolution.x << "," << volume_resolution.y << "," << volume_resolution.z
            << "    " << std::endl;
        out << "mu: " << mu << std::endl;
    ...
    Configuration(unsigned int argc, char** argv) {
        ...
        while ((c = getopt_long(argc, argv, short_options.c_str(), long_options, &option_index)) != -1)
            switch (c) {
                case 'u': {  // -u heuristic
                    std::cerr << "[UT] Control heuristic: " << optarg << std::endl;
                    this->control = optarg;
                    break;
                }
                case 'w': {  // -w feature_detection
                    this->features = atoi(optarg);
                    break;
                }
                case 'x': {  // -x extrapolation
                    this->extrapolate = atoi(optarg);
                    break;
                }
                case 'b':
                ...
                case 'c':  //   -c  (--compute-size-ratio)
                    ...
                    std::cerr << "update compute_size_ratio to " << this->compute_size_ratio << std::endl;
                    if ((this->compute_size_ratio != 1) && (this->compute_size_ratio != 2) &&
                        (this->compute_size_ratio != 4) && (this->compute_size_ratio != 8)) {
                        std::cerr << "ERROR: --compute-size-ratio (-c) must be 1, 2 ,4 or 8  (was " << optarg << ")\n";
                        flagErr++;
                    }
                    break;
                case 'd':
                    this->dump_volume_file = optarg;
                    std::cerr << "update dump_volume_file to " << this->dump_volume_file << std::endl;
                    break;
                case 'f':  //   -f  (--fps)
                ...
                case 'i':  //   -i  (--input-file)
                    ...
                    std::cerr << "update input_file to " << this->input_file << std::endl;
                    struct stat st;
                    if (stat(this->input_file.c_str(), &st) != 0) {
                        std::cerr << "ERROR: --input-file (-i) does not exist (was " << this->input_file << ")\n";
                        flagErr++;
                    }
                    break;
                case 'k':  //   -k  (--camera)
                    ...
                    std::cerr << "update camera to " << this->camera.x << "," << this->camera.y << "," << this->camera.z
                              << "," << this->camera.w << std::endl;
                    break;
                case 'o':  //   -o  (--log-file)
                    this->log_file = optarg;
                    std::cerr << "update log_file to " << this->log_file << std::endl;
                    break;
                case 'l':  //   -l (--icp-threshold)
                    this->icp_threshold = atof(optarg);
                    std::cerr << "update icp_threshold to " << this->icp_threshold << std::endl;
                    break;
                case 'm':  // -m  (--mu)
                    this->mu = atof(optarg);
                ...
                case 'p':  //   -p  (--init-pose)
                    ...
                    std::cerr << "update init_poseFactors to " << this->initial_pos_factor.x << ","
                              << this->initial_pos_factor.y << "," << this->initial_pos_factor.z << std::endl;
                    break;
                case 'q':
                    this->no_gui = true;
                    break;
                case 'r':  //   -r  (--integration-rate)
                    ...
                    std::cerr << "update integration_rate to " << this->integration_rate << std::endl;
                ...
                case 's':  //   -s  (--map-size)
                    ...
                    std::cerr << "update map_size to " << this->volume_size.x << "mx" << this->volume_size.y << "mx"
                              << this->volume_size.z << "m" << std::endl;
                    if ((this->volume_size.x <= 0) || (this->volume_size.y <= 0) || (this->volume_size.z <= 0)) {
                        std::cerr << "ERROR: --volume-size (-s) all dimensions must > 0 (was " << optarg << ")\n";
                        flagErr++;
                    }
                    break;
                case 't':  //   -t  (--tracking-rate)
                    this->tracking_rate = atof(optarg);
                    std::cerr << "update tracking_rate to " << this->tracking_rate << std::endl;
                    break;
                case 'z':  //   -z  (--rendering-rate)
                    this->rendering_rate = atof(optarg);
                    std::cerr << "update rendering_rate to " << this->rendering_rate << std::endl;
                    break;
                case 'v':  //   -v  (--volumetric-size)
                    this->volume_resolution = atoi3(optarg);
                    std::cerr << "update volumetric_size to " << this->volume_resolution.x << "x"
                              << this->volume_resolution.y << "x" << this->volume_resolution.z << std::endl;
                    if ((this->volume_resolution.x <= 0) || (this->volume_resolution.y <= 0) ||
                ...
                case 'y': {
                    ...
                    std::cerr << "update pyramid levels to " << pyramid2str(pyramid) << std::endl;
                    break;
                case 0:
                case '?':
                    std::cerr << "Unknown option character -" << char(optopt) << " or bad usage.\n";
                    print_arguments();
                    exit(0);
                default:
        ...
        if (flagErr) {
            std::cerr << "Exited due to " << flagErr << " error" << (flagErr == 1 ? "" : "s")

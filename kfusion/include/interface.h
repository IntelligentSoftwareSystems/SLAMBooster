...
enum ReaderType { READER_RAW, READER_SCENE, READER_OPENNI };

class DepthReader {
  public:
    virtual ~DepthReader() {}
    virtual bool readNextDepthFrame(float* depthMap) = 0;
    inline bool readNextDepthFrame(unsigned short int* UintdepthMap) { return readNextDepthFrame(NULL, UintdepthMap); }
    ...
    bool isValid() { return cameraOpen; }
    int getFrameNumber() { return (_frame); }
...
#endif
        double current_frame = clockData.tv_sec + clockData.tv_nsec / 1000000000.0;
...
static const float SceneK[3][3] = {{481.20, 0.00, 319.50}, {0.00, -480.00, 239.50}, {0.00, 0.00, 1.00}};
...
class SceneDepthReader : public DepthReader {
  ...
  public:
    ...
    ReaderType getType() { return (READER_SCENE); }
    inline float4 getK() { return make_float4(481.20, 480.00, 319.50, 239.50); }
    inline uint2 getinputSize() { return _size; }
    inline void restart() { _frame = 0; }
    ...
    // UT: This is where the depth values are processed
    inline bool readNextDepthFrame(float* depthMap) {
        std::ostringstream filename;
        get_next_frame();
        filename << this->_dir << "/scene_00_" << std::setfill('0') << std::setw(4) << _frame << ".depth";
        ...
        // UT: Changes
        // std::cerr << "Scene height:" << _sceneheight << " scene width:" << _scenewidth << "\n";
        ...
        for (int v = 0; v < _sceneheight; v++) {
            for (int u = 0; u < _scenewidth; u++) {
                ...
                depthMap[u + v * _scenewidth] =
                    depthMap[u + v * _scenewidth] / std::sqrt(u_u0_by_fx * u_u0_by_fx + v_v0_by_fy * v_v0_by_fy + 1);
            }
        }
        return index > 0;
    }
...
class RawDepthReader : public DepthReader {
  ...
  public:
    ...
    ReaderType getType() { return (READER_RAW); }
    inline bool readNextDepthFrame(uchar3* raw_rgb, unsigned short int* depthMap) {
...
#else
        unsigned int size_of_frame = (sizeof(unsigned int) * 4 + _size.x * _size.y * sizeof(unsigned short int) +
        ...
        if (depthMap) {
            ...
            total += fread(depthMap, sizeof(unsigned short int), newImageSize[0] * newImageSize[1], _pFile);
            expected_size += 1 + newImageSize[0] * newImageSize[1];
        } else {
            total += fread(&(newImageSize), sizeof(newImageSize), 1, _pFile);
            total += newImageSize[0] * newImageSize[1];
            fseek(_pFile, newImageSize[0] * newImageSize[1] * sizeof(unsigned short int), SEEK_CUR);
            expected_size += 1 + newImageSize[0] * newImageSize[1];
        }
        ...
        if (raw_rgb) {
            total += fread(&(newImageSize), sizeof(newImageSize), 1, _pFile);
            total += fread(raw_rgb, sizeof(uchar3), newImageSize[0] * newImageSize[1], _pFile);
            expected_size += 1 + newImageSize[0] * newImageSize[1];
        } else {
            total += fread(&(newImageSize), sizeof(newImageSize), 1, _pFile);
            total += newImageSize[0] * newImageSize[1];
            fseek(_pFile, newImageSize[0] * newImageSize[1] * sizeof(uchar3), SEEK_CUR);
            expected_size += 1 + newImageSize[0] * newImageSize[1];
        }
#endif

        if (total != expected_size) {
            std::cout << "End of file" << (total == 0 ? "" : "(garbage found)") << "." << std::endl;
            return false;
        } else {
            return true;
        }
    ...
    inline uint2 getinputSize() { return _size; }
    inline float4 getK() { return make_float4(531.15, 531.15, 640 / 2, 480 / 2); }
};
...
class MyDepthFrameAllocator : public openni::VideoStream::FrameAllocator {
  ...
  public:
    MyDepthFrameAllocator(uint16_t* depth_buffers) : depth_buffers_(depth_buffers) {}
...
class OpenNIDepthReader : public DepthReader {
  ...
  public:
    ...
    ReaderType getType() { return (READER_OPENNI); }
    OpenNIDepthReader(std::string filename, int fps, bool blocking_read)
        ...
        if (colorMode.getResolutionX() != _size.x || colorMode.getResolutionY() != _size.y) {
            std::cout << "Incorrect rgb resolution: " << colorMode.getResolutionX() << " " << colorMode.getResolutionY()
                      << std::endl;
            // exit(3);
            return;
        }
        ...
        if (device.isFile()) {
            ...
            depthFrame.release();
            colorFrame.release();
            device.getPlaybackControl()->setSpeed(-1);  // Set the playback in a manual
                                                        // mode i.e. read a frame
    ...
    inline uint2 getinputSize() { return _size; }
    inline float4 getK() { return make_float4(481.2, 480, 640 / 2, 480 / 2); }
};
#else
class OpenNIDepthReader : public DepthReader {
  public:
...
class OpenNI15DepthReader : public DepthReader {
  ...
  public:
    ...
    OpenNI15DepthReader(std::string filename, int fps, bool blocking_read) : DepthReader() {

...
/* Turn half precision feature on */
...
// UT
__kernel void computationSizeUpKernel(
        __global float * new_vertex,
        __global float * new_normal,
    ...
    vstore3(vload3(pixel.x/2 + inSize.x * (pixel.y/2), vertex), pixel.x + pixel.y * outSize.x, new_vertex);
    vstore3(vload3(pixel.x/2 + inSize.x * (pixel.y/2), normal), pixel.x + pixel.y * outSize.x, new_normal);
}


__kernel void computationSizeDownKernel(
        __global float * new_vertex,
        __global float * new_normal,
        __global float * vertex,
        __global float * normal,
    ...
    float3 t_vertex = (float3)(0.0, 0.0, 0.0);
    t_vertex += vload3(centerPixel.x + inSize.x * centerPixel.y, vertex);
    t_vertex += vload3(centerPixel.x + 1 + inSize.x * (centerPixel.y + 1), vertex);
    t_vertex += vload3(centerPixel.x + inSize.x * centerPixel.y, vertex);
    t_vertex += vload3(centerPixel.x + 1 + inSize.x * (centerPixel.y + 1), vertex);

    float3 t_normal = (float3)(0.0, 0.0, 0.0);
    t_normal += vload3(centerPixel.x + inSize.x * centerPixel.y, normal);
    t_normal += vload3(centerPixel.x + 1 + inSize.x * (centerPixel.y + 1), normal);
    t_normal += vload3(centerPixel.x + inSize.x * centerPixel.y, normal);
    t_normal += vload3(centerPixel.x + 1 + inSize.x * (centerPixel.y + 1), normal);

    vstore3(t_vertex/4.0f, pixel.x + pixel.y * outSize.x, new_vertex);
    vstore3(t_normal/4.0f, pixel.x + pixel.y * outSize.x, new_normal);
}


__kernel void DepthAVGKernel(
        __global float * avg_out,
        __global float * dep,
        const uint2 inSize
        ) {
    uint2 pixel = (uint2) (get_global_id(0), get_global_id(1));
    ...
    uint2 h_range;
    uint v_index;
    uint skip = 640/inSize.x;
    uint2 workSize = (uint2) (4, 24);
    float4 h_border = (float4)(0.1, 0.5, 0.5, 0.9);
    float4 v_border = (float4)(0.1, 0.5, 0.5, 0.9);
    switch(pixel.x) {
        case 0: { //tl
                    h_range = (uint2) (h_border.x * inSize.x, h_border.y * inSize.x);
                    v_index = v_border.x * inSize.y + pixel.y * skip;
                    break;
                }
        case 1: { //tr
                    h_range = (uint2) (h_border.z * inSize.x, h_border.w * inSize.x);
                    v_index = v_border.x * inSize.y + pixel.y * skip;
                    break;
                }
        case 2: { //bl
                    h_range = (uint2) (h_border.x * inSize.x, h_border.y * inSize.x);
                    v_index = v_border.z * inSize.y + pixel.y * skip;
                    break;
                }
        case 3: { //br
                    h_range = (uint2) (h_border.z * inSize.x, h_border.w * inSize.x);
        ...
        default: printf("disaster avg\n");
    }

    uint counter = 0;
    float line_sum = 0;
    for (int i = h_range.x; i < h_range.y; i += skip) {
        float val = dep[inSize.x * v_index + i];
        if (val != 0) {
            counter++;
            line_sum += val;
        }
    ...
    vstore2((float2)(line_sum, counter), pixel.x + pixel.y * workSize.x, avg_out);
}


__kernel void DepthSTDKernel(
        __global float * std_out,
        __global float * avg_in,
        __global float * dep,
        const uint2 inSize
        ) {
    uint2 pixel = (uint2) (get_global_id(0), get_global_id(1));
    ...
    uint skip = 640/inSize.x;
    uint2 workSize = (uint2) (4, 24);
    for (int i = 0; i < workSize.y; i++) {
        float2 partial_sum = vload2(pixel.x + workSize.x * i, avg_in);
        total_sum += partial_sum.x;
        counter += partial_sum.y;
    }
    float avg = ((uint)counter)?(total_sum/counter):0;
    ...
    vstore2((float2)(sum, counter), pixel.x + pixel.y * workSize.x, std_out);
}
...
__kernel void DepthAVGKernel(
    ...
    switch(pixel.x) {
        case 0: { //tl
                    h_range = (uint2) (0.2*inSize.x, 0.4*inSize.x);
                    v_index = 0.2*inSize.y + pixel.y;
                    break;
                }
        case 1: { //tr
                    h_range = (uint2) (0.6*inSize.x, 0.8*inSize.x);
                    v_index = 0.2*inSize.y + pixel.y;
                    break;
                }
        case 2: { //bl
                    h_range = (uint2) (0.2*inSize.x, 0.4*inSize.x);
                    v_index = 0.6*inSize.y + pixel.y;
                    break;
                }
        case 3: { //br
                    h_range = (uint2) (0.6*inSize.x, 0.8*inSize.x);
        ...
        case 4: { //mid
                    h_range = (uint2) (0.4*inSize.x, 0.6*inSize.x);
                    v_index = 0.4*inSize.y + pixel.y;
                    break;
                }
        default: printf("disaster avg\n");
    }
    ...
    uint counter = 0;
    float sum = 0;
    for (int i = h_range.x; i < h_range.y; i++) {
        float val = dep[inSize.x * v_index + i];
        if (val != 0) {
            counter++;
            sum += val;
    ...
    uint2 workSize = (uint2) (5, inSize.y*0.2);
    vstore2((float2)(sum, counter), pixel.x + pixel.y * workSize.x, avg_out);
}

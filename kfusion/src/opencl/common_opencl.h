...
// UT: Added these defintions to allow half-precision computations
/* ---- cl_halfn ---- */
typedef union {
...
#define RELEASE_OUT_BUFFER(name)                                                                   \
    clError = clEnqueueReadBuffer(commandQueue, name##Buffer, CL_TRUE, 0, name##BufferSize,        \
                                  name##Ptr, 0, NULL, NULL);                                       \
...
#define RELEASE_KERNEL(kernelname)                                                                 \
    clError = clReleaseKernel(kernelname);                                                         \
    checkErr(clError, "clReleaseKernel");

#define CREATE_KERNEL(name) CREATE_KERNELVAR(name, #name);
#define CREATE_KERNELVAR(varname, kernelname)                                                      \
    cl_kernel varname = clCreateKernel(program, kernelname, &clError);                             \
...
#define CREATE_OUT_BUFFER(name, ptr, size)                                                         \
    ...
    cl_mem name##Buffer =                                                                          \
        clCreateBuffer(context, CL_MEM_WRITE_ONLY, name##BufferSize, NULL, &clError);              \
    checkErr(clError, "clCreateBuffer input");

#define CREATE_IN_BUFFER(name, ptr, size)                                                          \
    size_t name##BufferSize = size;                                                                \
    void* name##Ptr = (void*)ptr;                                                                  \
    cl_mem name##Buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,          \
                                         name##BufferSize, name##Ptr, &clError);                   \
    checkErr(clError, "clCreateBuffer input");
...
#define CREATE_INOUT_BUFFER(name, ptr, size)                                                       \
    ...
    void* name##Ptr = (void*)ptr;                                                                  \
    cl_mem name##Buffer =                                                                          \
        clCreateBuffer(context, CL_MEM_READ_WRITE, name##BufferSize, NULL, &clError);              \
    checkErr(clError, "clCreateBuffer input");                                                     \
    clError = clEnqueueWriteBuffer(commandQueue, name##Buffer, CL_TRUE, 0, name##BufferSize, ptr,  \
                                   0, NULL, NULL);                                                 \

...
public:
    ...
    void compareVolume(__device_builtin__uchar4 * out,
			const __device_builtin__uint2 outputSize, int, int,
			__device_builtin__float4 k, float largestep, const char* ref_file);
  ...
  void updateComputationSize(__device_builtin__uint2 cSize); // UT:
};
...
void clean() {
	...
	// UT: FIXME: Need to implement this. I have added this to just allow compilation.
...
// UT:
void Kfusion::updateComputationSize(__device_builtin__uint2 cSize) {
    return;
}

void Kfusion::compareVolume(__device_builtin__uchar4 * out,
	__device_builtin__uint2 outputSize, int frame, int rate,
	__device_builtin__float4 k, float largestep, const char* ref_file) {
